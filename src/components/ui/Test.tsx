      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-24 relative z-10">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-${numberOfColumn} lg:grid-cols-${numberOfColumn} gap-4 w-[90%]  mx-auto ${customContainerClass ? customContainerClass : ''}`}>
          {content.map((feature, index) => {
            ...
            >
              {/* Top Accent Line */}
              <div className={`absolute top-0 left-0 w-full h-1 ${stylesList[index].color.replace('bg-', 'bg-linear-to-r from-transparent to-')}`}></div>
              
              {/* Icon Container */}
              <div className='w-full flex gap-4 items-start '>
                 <div className={`w-16 h-16 aspect-square rounded-2xl ${stylesList[index].color} flex items-center justify-center  shadow-md transform group-hover:rotate-6 transition-transform duration-300`}>
                {stylesList[index].icon}
              ...
              <div className={`absolute -bottom-4 -right-4 w-20 h-20 rounded-full ${stylesList[index].color} opacity-5 group-hover:scale-150 transition-transform duration-500`}></div>
            </div>
          )})}
        </div>
